...
public class Bbox implements Cloneable {
    ...
    public String getWgs84LowerCorner() {
        ...
        for (int i = 0; i < getDimensionality(); i++) {
            if (i > 0) output += " ";
            if (getType(i).equals(AxisTypes.X_AXIS)) output += getWgs84minLon();
            else if (getType(i).equals(AxisTypes.Y_AXIS)) output += getWgs84minLat();
            else {
                tmp = getMinValue(i);
                // Fill possible space between date and time in timestamp with "T" (ISO:8601)
                // Disable: breaks XML schema (requires xs:double)
    ...
    public String getWgs84UpperCorner() {
        ...
        for (int i = 0; i < getDimensionality(); i++) {
            if (i > 0) output += " ";
            if (getType(i).equals(AxisTypes.X_AXIS)) output += getWgs84maxLon();
            else if (getType(i).equals(AxisTypes.Y_AXIS)) output += getWgs84maxLat();
            else {
                tmp = getMaxValue(i);
                // Fill possible space between date and time in timestamp with "T" (ISO:8601)
                // Disable: breaks XML schema (requires xs:double)
